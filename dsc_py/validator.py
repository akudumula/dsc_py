...
def proof_of_memory(thread_id):
    ...
    while(hash_block_thread.nonce<hashes_to_be_generated):
        ...
        proof_of_memory_hashes[hash_block_thread.nonce] =(hash,hash_block_thread.nonce)
        hash_block_thread.nonce+=threads
...
def proof_of_memory_search():
    ...
    for hash in proof_of_memory_hashes :
        if isinstance(hash,tuple):
            if compare_first_bits(hash[0],result_hash,difficulty) == 1:
                nonce = hash[1]
                break
...
def connect_to_metronome():
    ...
    global nonce
    global hashes_generated
    if output["validator1"]["proof_pow"]["enable"] == True:
        log_message("DSC v1.0")
        fingerprint = output["validator1"]["fingerprint"]
        public_key = output["validator1"]["public_key"]
        threads = output["validator1"]["proof_pow"]["threads_hash"]
        ...
        while True:
            ...
            if (validator_win_response.validator_win == 1):
                ...
                for transactions in transaction_list:
                    transactions_list_validator.append(metronome_pb2.transactionMetronome(sender_public_address = transactions.sender_public_address,recepeint_pubic_address =transactions.recepeint_pubic_address,value=transactions.value,timestamp = transactions.timestamp,transaction_id = transactions.transaction_id,signature = transactions.signature ))
                ...
                validator_confirm_request = pool_pb2.validator_confirm()
                transactions_list_pool = []
                for transactions in transaction_list:
                    validator_confirm_request.transaction_ids.append(transactions.transaction_id)
                
                # validator_confirm = pool_pb2.validator_confirm(transaction_ids = transactions_list_pool)
                a = stub_pool.move_to_confirm(validator_confirm_request)
        # print(metronome_response)
                    
    elif output["validator1"]["proof_pom"]["enable"] == True:
        log_message("DSC v1.0")
        fingerprint = output["validator1"]["fingerprint"]
        public_key = output["validator1"]["public_key"]
        threads = output["validator1"]["proof_pom"]["threads_hash"]
        log_message(f"Proof of Memory ({threads}-threads)")
        log_message(f"Fingerprint: {fingerprint}")
        global proof_of_memory_hashes
        size = convert_to_bytes(output["validator1"]["proof_pom"]["memory"])
